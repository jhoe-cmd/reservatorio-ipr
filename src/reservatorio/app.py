    ...
    if not dados_validos:
        ...
        with st.spinner("Processando otimização e acoplamento térmico..."):
            try:
                ...
                # --- DIAGNÓSTICO 3D ---
                st.markdown("---")
                st.subheader("🔍 Diagnóstico de Incerteza Numérica")
                ...
                with st.spinner("Mapeando topografia de erro tridimensional..."):
                    ...
                    with col_diag1:
                        if diag["area_incerteza_pct"] < 5.0:
                            st.success(f"✅ **Área de Incerteza:** {diag['area_incerteza_pct']:.1f}%")
                        else:
                            st.warning(f"⚠️ **Área de Incerteza:** {diag['area_incerteza_pct']:.1f}%")
                            
                    with col_diag2:
                        if diag["condicionamento_ci"] < 10:
                            st.success(f"✅ **Condicionamento (CI):** {diag['condicionamento_ci']:.1f}")
                        else:
                            st.warning(f"⚠️ **Condicionamento (CI):** {diag['condicionamento_ci']:.1f}")
                    ...
                    fig_3d.update_layout(scene=dict(xaxis_title=label_x, yaxis_title=label_y, zaxis_title='RMSE'), height=550)
                    st.plotly_chart(fig_3d, use_container_width=True)

                # --- TORNADO ---
                st.markdown("### 🌪️ Análise de Sensibilidade Estatística")
                impacto_p1 = 0.45 if is_fetkovich else 0.40
                impacto_p2 = 0.55 if is_fetkovich else 0.60
                
                fig_tornado = go.Figure(go.Bar(
                    x=[impacto_p2 * 100, 15.0, impacto_p1 * 100], y=[label_y, 'Pe', label_x], orientation='h',
                    marker=dict(color=['#3182ce', '#3182ce', '#3182ce']),
                    text=[f"{v:.1f}%" for v in [impacto_p2 * 100, 15.0, impacto_p1 * 100]], textposition='auto'
                ))
                fig_tornado.update_layout(title="Contribuição de cada parâmetro na incerteza", height=300)
                ...
                tex_base = (
                    ...
                    "\\usepackage{amsmath}\n"
                    "\\begin{document}\n\n"
                    f"\\section*{{Memorial de Calculo Termodinâmico - {well_name}}}\n\n"
                    "\\subsection*{1. Ajuste Historico (Modelo Base)}\n"
                    "\\begin{itemize}\n"
                    f"  \\item Erro Residual (RMSE): {str_rmse} psi\n"
                    f"  \\item Parâmetro 1 Calibrado: {str_j}\n"
                    f"  \\item Parâmetro 2 Calibrado: {str_p}\n"
                    "\\end{itemize}\n\n"
                )
                ...
                if ativar_termico:
                    ...
                    tex_termico = (
                        ...
                        "\\begin{itemize}\n"
                        f"  \\item Temperatura de Referência: {t_ref} C\n"
                        f"  \\item Temperatura do Reservatório: {t_res} C\n"
                        f"  \\item Multiplicador Dinâmico de Desempenho: {str_mult}\n"
                        f"  \\item AOF Original (Isotérmico): {str_aof} {unidade_vazao}\n"
                        f"  \\item AOF Corrigido (Térmico): {str_aof_t} {unidade_vazao}\n"
                        f"  \\item Ganho de Produção Estimado: {str_delta} {unidade_vazao}\n"
                        "\\end{itemize}\n\n"
                    )
                else:
